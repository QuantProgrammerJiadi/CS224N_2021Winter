...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    loss = -np.log(softmax(np.matmul(outsideVectors, centerWordVec)))[outsideWordIdx]  #1*1
    y = np.zeros(shape = (outsideVectors.shape[0],))   #n*1
    y[outsideWordIdx] = 1
    y_hat = softmax(np.matmul(outsideVectors, centerWordVec)) #n*1
    gradCenterVec = np.matmul(outsideVectors.T, y_hat - y)  #m*1
    gradOutsideVecs = np.matmul((y_hat - y).reshape((-1, 1)), centerWordVec.reshape((1, -1)))  #n*m
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    negativematrix = outsideVectors[negSampleWordIndices]  #K*m
    loss = -np.log(sigmoid(np.matmul(outsideVectors[outsideWordIdx], centerWordVec))) - np.sum(np.log(sigmoid(-np.matmul(negativematrix, centerWordVec))))
    gradCenterVec = -(1-sigmoid(np.matmul(outsideVectors[outsideWordIdx], centerWordVec)))*outsideVectors[outsideWordIdx].T +\
                    np.matmul(negativematrix.T, 1-sigmoid(-np.matmul(negativematrix,centerWordVec)))  #m*1
    indice_counter = Counter(indices)
    gradOutsideVecs = np.zeros(shape = outsideVectors.shape)  #n*m
    for i in indices:
        if i == outsideWordIdx:
            grad_vec = (sigmoid(np.matmul(outsideVectors[outsideWordIdx], centerWordVec))-1)*centerWordVec #m*1
        else:
            cnt = indice_counter[i]
            grad_vec = cnt*(1-sigmoid(np.matmul(-outsideVectors[i], centerWordVec)))*centerWordVec #m*1
        gradOutsideVecs[i] = grad_vec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerWordVectors -- center word vectors (as rows) is in shape 
                        ...
                        for all words in vocab (V in pdf handout), i.e. (n,m)
    outsideVectors -- outside vectors is in shape 
                        (num words in vocab, word vector length) 
                        for all words in vocab (transpose of U in the pdf handout), i.e. (n,m)
    ...
    for word in outsideWords:
        ...
        loss_c, gradCenterVec_c, gradOutsideVecs_c = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset,)
        loss += loss_c
        gradCenterVecs[center_idx] += gradCenterVec_c
        gradOutsideVectors += gradOutsideVecs_c
    ### END YOUR CODE
