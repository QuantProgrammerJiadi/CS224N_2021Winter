...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(1, self.hidden_size), a = 0.0, b = 1.0))
        self.dropout = nn.Dropout(self.dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.hidden_size, self.n_classes),
                                                                            gain=nn.init.calculate_gain('relu')))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(torch.empty(1, self.n_classes), a = 0.0, b = 1.0))
        ### END YOUR CODE
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, torch.flatten(w)).view(w.size()[0], self.n_features * self.embed_size)
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        relu = nn.ReLU()
        h = relu(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)  #(batch_size*hidden_size)
