...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        #Encoding:
        src_emb = self.model_embeddings.source(source_padded) #X (src_len, b, e)
        src_emb = pack_padded_sequence(input = src_emb, lengths = source_lengths) #把tensor变成PackedSequence
        enc_hiddens, (last_hidden, last_cell) = self.encoder(src_emb)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first = True) #(b, src_len, h*2)
        #Decoding:
        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), dim = 1))  #0和1分别表示forward, backward
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = self.decoder(Ybar_t, dec_state) #(tuple(Tensor, Tensor)), ((b,h), (b,h))
        dec_hidden, dec_cell = dec_state #((b,h), (b,h))
        enc_hiddens_proj = enc_hiddens_proj.transpose(2, 1) #shape: (b, h, src_len)
        e_t = torch.bmm(dec_hidden.unsqueeze(-2), enc_hiddens_proj).squeeze(-2)  #(b,1,h)*(b,h,src_len)->(b, 1, src_len)->(b, src_len)
        ### END YOUR CODE
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(-2), enc_hiddens).squeeze(-2)  #(b, 1, src_len)*(b, src_len, 2h)->(b, 2h)
        U_t = torch.cat([a_t, dec_hidden], dim = 1)  #dec_hidden: (b,h)
